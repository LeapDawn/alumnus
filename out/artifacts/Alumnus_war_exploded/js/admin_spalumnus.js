...
M_table.getList = function (curr) {
	var newURL = M_table.url + url.admin_spalumnus_list;
	var param = {
		key:  {
	...
	publicDom.getData('post',newURL, jsonStr,function(JsonData){
		if(JsonData.code == 0){
			...
			M_table.showConfirmModal('错误','success','获取特殊校友信息列表失败！');
		}		
	})
}
...
M_table.createList = function (list) {
	...
	$('.btn-detail').click(function(){
		...
		localStorage.setItem('spalumnus_info_id', id);
		location.href="admin_spalumnus_detail.html";
	});
...
M_table.deleteItem = function(id) {
	var newURL = M_table.url + url.spalumnus_deleteion + "?id=" + id;
	publicDom.getData('delete',newURL, null, function(JsonData){
		if(JsonData.code == 0){
			M_table.showConfirmModal("成功", "success", "成功删除特殊校友信息!");
			location.reload();
		} else {
			M_table.showConfirmModal("失败", "", "删除失败!该特殊校友信息不允许被删除!");
		}
	})
