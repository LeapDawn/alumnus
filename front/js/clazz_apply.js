...
M_table.getList = function (curr) {
	var newURL = M_table.url + url.clazz_apply_list;
	var param = {
		key:0,
		page: curr || 1,
		rows: 5
	...
	publicDom.getData('post',newURL, jsonStr,function(JsonData){
		if(JsonData.code == 0){
			...
			M_table.showConfirmModal('错误','success','获取校友班级申请列表失败！');
		}		
	})
}
	...
	for( var i = 0 ; i < list.length ; i ++ ){
		var tableItem = '<div class="col-md-8">\
		  			<div class="content-box-large">\
		  				...
		  				<div data-id="' +list[i].id+'" class="panel-body">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+ list[i].content +'\
		  					<br><br><br><br>\
		  					<button type="button" class="btn btn-primary col-sm-offset-9 btn-approve">通过</button>\
							<button type="button" class="btn btn-danger pull-right btn-disapprove">否决</button>\
		  				</div>\
		  			</div>\
		  		</div>';
...
M_table.approveApply = function(id) {
	var newURL = M_table.url + url.clazz_apply_approve;
	var param = {
		id:id
	}
	var jsonStr = JSON.stringify(param);
...
M_table.disapproveApply = function(id) {
	var newURL = M_table.url + url.clazz_apply_disapprove;
	var param = {
		id:id
	}
	var jsonStr = JSON.stringify(param);
