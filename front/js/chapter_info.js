...
M_table.init = function () {
	// 获取列表页传递过来的ID
	M_table.id = localStorage.getItem('chapter_name');
...
M_table.getItem = function(id) {
	var newURL = M_table.url + url.chapter_item + "?item=" + M_table.id;
	publicDom.getData('get',newURL, null,function(data){
		if(data.code==0){
			var value = data.value;
			$(".chapter_detail").text(value.name),
			$("#chapter_content").text(value.content)
		}else{
...
M_table.updateItem = function(id){
	var newURL = M_table.url + url.admin_alumnus_update;
	var param = {
		id:M_table.id,
		name:$("#name").val(),
