	...
	publicDom.getData('get',newURL, null,function(data){
		if(data.code == 0){
			if (data.value == null) {
				...
				$('.input-group').append(
						...
						<div class="tools-rt">\
							<span class="input-group-btn">\
		                      <button id="apply" class="btn btn-primary" type="button">加入分会</button>\
		                    </span>\
		                    <a href="chapter_addition.html">\
		                   	  <button class="btn btn-primary" type="button">创建分会</button>\
		                    </a>\
						</div>');
				...
				$('.input-group').append(
						'<div class="tools-rt">\
		                    <a href="mychapter.html">\
		                   	  <button class="btn btn-primary" type="button">我的分会</button>\
		                    </a>\
						</div>');
				$('#apply').click(function(){
					C_table.readyApply();
